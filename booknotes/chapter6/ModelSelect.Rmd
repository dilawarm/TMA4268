...
and a very nice way of distributing analysis. It has some very simple syntax rules.
...
------------------------
We will now use the pacage `leaps` to evaluate all the best-subset models.
``` {r}
library(leaps)
...
---------------------------------------
Lets make a training and validation set, so that we can choose a good model.
...
We know there are 19 models, so we set up some vectors to record the errors. We have to do a bit of work here, because there is no predict method for `regsubsets`.
```{r}
...
Now we will fit a lasso model; for this we use the default `alpha=1`
```{r}
